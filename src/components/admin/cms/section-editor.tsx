...
function HeroEditor({ content, patch }: { content: SiteContent; patch: PatchFn }) {
  ...
  const set = (key: keyof typeof h, val: string | string[]) =>
  ...
  return (
    <div className="space-y-6">
      <SectionBlock title="Cabeçalho">
        ...
        <Field label="Emoji digitais"><Input value={s.digitalEmoji} onChange={(v) => patch((c) => ({ ...c, services: { ...c.services, digitalEmoji: v } }))} /></Field>
        <Field label="Título digitais"><Input value={s.digitalTitle} onChange={(v) => patch((c) => ({ ...c, services: { ...c.services, digitalTitle: v } }))} /></Field>
        <Field label="Emoji técnico"><Input value={s.techEmoji} onChange={(v) => patch((c) => ({ ...c, services: { ...c.services, techEmoji: v } }))} /></Field>
      ...
      <div className="space-y-4">
        {p.items.map((item, i) => (
          <CardBox key={i} title="Projeto" index={i} onRemove={() => patch((c) => ({ ...c, portfolio: { ...c.portfolio, items: c.portfolio.items.filter((_, idx) => idx !== i) } }))}>
            ...
            <Field label="Imagem (URL)"><Input value={item.image} onChange={(v) => patch((c) => { const items = [...c.portfolio.items]; items[i] = { ...items[i], image: v }; return { ...c, portfolio: { ...c.portfolio, items } }; })} /></Field>
        ...
        <AddButton label="Adicionar projeto" onClick={() => patch((c) => ({ ...c, portfolio: { ...c.portfolio, items: [...c.portfolio.items, { name: "Novo projeto", client: "", category: "site", description: "", image: "/placeholder.jpg", stack: [], year: "2026", stats: { users: "—", modules: "—" }, features: [] }] } }))} />
      </div>
    </div>
  );
}
      ...
      <div className="space-y-4">
        {hs.items.map((item, i) => (
          <CardBox key={i} title="Card serviço" index={i} onRemove={() => patch((c) => ({ ...c, ui: { ...c.ui, homeServices: { ...c.ui.homeServices, items: c.ui.homeServices.items.filter((_, idx) => idx !== i) } } }))}>
            ...
            <Field label="Link (opcional)"><Input value={item.href ?? ""} onChange={(v) => patch((c) => { const items = [...c.ui.homeServices.items]; items[i] = { ...items[i], href: v || undefined }; return { ...c, ui: { ...c.ui, homeServices: { ...c.ui.homeServices, items } } }; })} /></Field>
          </CardBox>
        ))}

...
export function BusinessCompanies() {
  ...
  return (
    <AdminPage
      title="Empresas"
      description="Cadastre clientes e empresas para vincular aos projetos Business."
    ...
    >
      <div className="biz-grid">
        <div className="biz-form">
          ...
          <Field label="Nome fantasia"><Input value={form.tradeName} onChange={(v) => set("tradeName", v)} /></Field>
          <ImageField label="Logo" value={form.logo} onChange={(v) => set("logo", v)} />
          <Field label="Descricao"><Textarea value={form.description} onChange={(v) => set("description", v)} rows={3} /></Field>
          <Field label="Site"><Input value={form.site} onChange={(v) => set("site", v)} /></Field>
          ...
          <Field label="Endereco"><Input value={form.address} onChange={(v) => set("address", v)} /></Field>
          <Field label="Instagram"><Input value={form.instagram} onChange={(v) => set("instagram", v)} /></Field>
          <Field label="LinkedIn"><Input value={form.linkedin} onChange={(v) => set("linkedin", v)} /></Field>
          <Field label="Observacoes"><Textarea value={form.notes} onChange={(v) => set("notes", v)} rows={3} /></Field>
          <div className="biz-actions">
            <button type="button" className="cms-btn cms-btn--primary" disabled={saving} onClick={() => void save()}>
              {saving ? "Salvando..." : "Salvar empresa"}
            </button>
            {editing ? (
              <button type="button" className="cms-btn cms-btn--ghost" onClick={() => { setEditing(null); setForm(empty); }}>
                Cancelar
              </button>
            ) : null}
          </div>
            ...
            <ul className="biz-list">
              {companies.map((item) => (
                <li key={item.id} className="biz-row">
                  ...
                  <button type="button" className="cms-btn cms-btn--ghost" onClick={() => { setEditing(item.id); setForm(item); }}>
                    Editar
                  </button>
